...
const verifyCurrentEmail = async () => {
  ...
  let res = null
  try {
    res = await api.post(`/users/check-email?email=${emailData.value.currentEmail}`, {
      headers: {
...
const sendCode = async () => {
  ...
  message.value = ''
  try {
    await api.post(`/users/code-change-email?&email=${emailData.value.newEmail}`)
  } catch (error) {
    message.value = error.response?.data?.message?.value
    isUpdated.value = false
    return
            ...
            <div v-if="activeTab === 'email'" class="row justify-content-center">
              <div class="col-lg-8">
                <div class="row g-4">
                  <!-- Current Email -->
                  <div class="col-12">
                    <label for="currentEmail" class="form-label form__label">
                      {{ t('profile.currentEmail') }}:</label
                    >
                  ...
                  <div class="col-12 mt-4" v-if="currentEmailVerified">
                    <label for="newEmail" class="form-label form__label">
                      {{ t('profile.newEmail') }}
                    </label>
                    <div class="input__wrapper">
